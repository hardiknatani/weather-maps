...
function getData(feature,content) {
    var _a;
    const layerData = [
        'layer',
    ...
    const featureData:any = ['properties'];
    // if (feature.id) {
...
export default function popupTemplate(features,properties) {
    let current = 0;
    ...
    const templateFeature = (feature) => {
        ...
        const data = getData(features[0],properties);
        data.forEach((record) => {
